...
tldr: 语音LLM通常将预训练语音编码器连接到LLM，但ASR编码器产生语言特定表示，而LLM使用语言无关空间，导致结构错位。本文提出利用语音翻译目标增强编码器预训练，强制模型学习跨语言统一表示。实验证明该方法改善跨模态集成，在下游语音LLM任务中性能显著提升，为语音LLM提供了有效的预训练策略。
source: arxiv
...
motivation: 解决语音编码器与LLM之间语言表示空间不匹配的问题。
method: 在语音编码器预训练中引入翻译目标，学习语言无关表示。
result: 翻译增强预训练提升跨模态集成，在多项下游语音LLM任务上取得更优性能。
conclusion: 语音翻译是对齐编码器与LLM的有效策略，显著改善语音LLM性能。
---

## 摘要
将预训练的语音编码器连接到大语言模型（LLM）是构建语音LLM的标准架构。然而，编码器与LLM之间存在结构上的不匹配。与通常在不同的语言特定空间中产生表示的基于自动语音识别的编码器不同，LLM在统一的与语言无关的空间中操作。需要一种机制来对齐编码器的语言特定表示与LLM的共享空间。我们认为语音翻译提供了一种原则性的方式来实现这一点。与单语言转录不同，翻译要求模型桥接不同语言并学习与语言无关的表示。我们通过实验评估了将翻译目标纳入语音编码器预训练的影响。结果表明，翻译增强的预训练改善了跨模态集成，并在下游语音LLM任务中带来了更优的性能。
